...
#' res = regfun_slos_fun(xfdlist,yfd,time_obs,yname=xnames[14],xnames)
#' }

     

regfun_slos_fun= function(xfdlist, yfd,   Lcholy , time_obs=seq(0,23,len=24), yname, xnames,
...
which_index = which(apply(abs(haty),2,max)>cutoff)
res_final = res_final%>%dplyr::slice(which_index)

return(list(estimated_fd =res_final, x=x, xname= xnames, transformy =y_transform , hatytransform = haty_transform, y=y,haty = haty, intery=intery, time=time_obs,AICc=AICc, targent_gene = yname,type=type,lambda=lambda,gamma=gamma,Lcholy = Lcholy))
